...
def get_knn_index(coor_q, coor_k=None): #Search K Near Points
    coor_k = coor_k if coor_k is not None else coor_q 
    # coor: bs, 3, np
    batch_size, _, num_points = coor_q.size()
    num_points_k = coor_k.size(2) + 1

    with torch.no_grad():
        idx = knn_point(8, coor_k.transpose(-1, -2).contiguous(), coor_q.transpose(-1, -2).contiguous()) # B G M
        idx = idx.transpose(-1, -2).contiguous()
        domain_query_idx = torch.full((batch_size,8,1),num_points_k-1, device = coor_q.device)
        idx = torch.cat([idx, domain_query_idx], dim =-1)
...
class PCTransformer(nn.Module):
    ...
    """
    def __init__(self, in_chans=3, embed_dim=768, depth=[6, 8], num_heads=6, mlp_ratio=2., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                        num_query = 224, knn_layer = -1,domain_enc = None, domain_dec = None):
        super().__init__()

        self.num_features = self.embed_dim = embed_dim
        
        self.knn_layer = knn_layer
        self.domain_enc = domain_enc
        self.domain_dec = domain_dec
        ...
        self.query_embed = nn.Embedding(num_query, embed_dim*2)
    ...
    def forward(self, inpc):
        ...
        #domain encoder prepare
        domain_enc = domain_enc.unsqueeze(1).expand(bs,1,-1)  
        domain_enc_pos, domain_enc_src =torch.split(domain_enc, self.embed_dim, dim=2)  
        ...
        #domain_query
        x=torch.cat([x, domain_enc_src], dim=1) 
        pos =torch.cat([pos, domain_enc_pos],dim=1) 
        inter_memory = []
        inter_domain_enc = []
        ...
        for i, blk in enumerate(self.encoder):
            ...
            if i < self.knn_layer:
                ...
                x_enc, domain_enc_out = torch.split(x,128,dim=1)
                inter_memory.append(x_enc)
                inter_domain_enc.append(domain_enc_out)
            else:
                x = blk(x + pos)
        ...
        #domain decoder prepare
        global_feature = self.increase_dim(memory.transpose(1,2)) 
        ...
        domain_dec = domain_dec.unsqueeze(0).expand(bs,-1,-1)   
        q = self.mlp_query(query_feature.transpose(1,2)).transpose(1,2) # B M C 
        domain_dec_pos, domain_dec_src = torch.split(domain_dec, self.embed_dim, dim=2) 

        query_embed = torch.cat([q, domain_dec_pos], dim=1) 
        memory = torch.cat([memory, domain_dec_src],dim=1)
        intermediate_dec = []

        # Decoder
        for i, blk in enumerate(self.decoder):
            if i < self.knn_layer:
                q = blk(query_embed, memory, new_knn_index, cross_knn_index)   # B M C  
                x_dec, domain_dec_out = torch.split(q, self.num_query, dim=1)   
                intermediate_query.append(x_dec)  
                intermediate_domain_dec.append(domain_dec_out)
                intermediate_dec.append(x_dec)
            else:
                q = blk(q, memory)
                x_dec, domain_dec_out = torch.split(q, self.num_query, dim=1)
                intermediate_dec.append(x_dec)  
        ...
        intermediate_query.append(x_dec)
        intermediate_domain_dec.append(domain_dec_out)
        output_dec = torch.stack(intermediate_dec) 

        intermediate_query = torch.stack(intermediate_query)    
        intermediate_domain_dec = torch.stack(intermediate_domain_dec)  

        return x_dec, coarse_point_cloud, inter_memory, inter_domain_enc, intermediate_query, intermediate_domain_dec, output_dec
